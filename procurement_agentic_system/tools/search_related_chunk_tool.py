...
class SearchRelatedChunk(BaseTool):
    ...
    description: str = (
        "Retrieves the top 5 semantically relevant chunks from both RFP and EA standard PDFs based on a query string. "
        "Useful for agents analyzing specific categories (e.g., Scope of Work, Core System Capabilities) to fetch relevant content "
        "for evaluation against industry best practices. The agent should only pass a query string as input."
    )
    
    embedding_manager: EmbeddingManager = Field(default_factory=lambda: EmbeddingManager(embedding_dim=768, db_path="embedding_db"))
